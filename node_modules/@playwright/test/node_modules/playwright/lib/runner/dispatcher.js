...
class JobDispatcher {
  ...
  _handleFatalErrors(errors) {
    const test = this._remainingByTestId.values().next().value;
    if (test) {
  ...
  _onDone(params) {
    ...
    for (const testId of params.skipTestsDueToSetupFailure) this._remainingByTestId.delete(testId);
    ...
    for (const test of this._job.tests) {
      if (!this._remainingByTestId.has(test.id)) ++test._runAttempts;
    }
    const remaining = [...this._remainingByTestId.values()];
    for (const test of retryCandidates) {
      if (test._runAttempts < test.retries + 1) remaining.push(test);
    }
