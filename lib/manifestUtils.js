...
const displayManifest = (manifest, imageId, viewMoreUrl) => {
  if (manifest) {
 ...
 */
export const getC2PAManifest = async (imageElement, singleImageVerification) => {
  const event = {};
  ...
  try {
    ...
    displayManifest(manifest, imgId, viewMoreUrl, singleImageVerification);
  } catch (error) {
    console.log(error, 'singleImgVerification value', singleImageVerification)
