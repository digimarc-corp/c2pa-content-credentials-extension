...
function prepareAndHandleVideo(videoElement) {
  let iconContainer = document.getElementById('icon-container');
  if (!iconContainer) {
    iconContainer = createIconContainer();
  }
...
function handleVideos() {
  ...
  const videoElements = Array.from(document.querySelectorAll('video')).filter((video) => !handledImages.includes(video.id));
