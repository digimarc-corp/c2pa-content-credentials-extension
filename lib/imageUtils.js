...
function prepareAndHandleAudio(audioElement, singleImageVerification) {
  let iconContainer = document.getElementById('icon-container');
  if (!iconContainer) {
    iconContainer = createIconContainer();
  }
  ...
  if (!audioElement.src) {
    ...
    setImageDataURI(audioElement).then((imageDataURI) => {
      ...
      getC2PAManifest(audioElement, addIconForImage, singleImageVerification);
    });
  }
}
...
export function handleSingleVideo(videoElement, singleImageVerification) {
  if (videoElement && !handledMedias.includes(videoElement.getAttribute('c2paId'))) {
    prepareAndHandleVideo(videoElement, singleImageVerification);
    visibleMedias.push(videoElement);
  }
}

export function handleSingleAudio(audioElement, singleImageVerification) {
  if (audioElement && !handledMedias.includes(audioElement.getAttribute('c2paId'))) {
    prepareAndHandleAudio(audioElement, singleImageVerification);
