...
export function handleSingleImage(imageElement, singleImageVerification) {
  if (imageElement && !handledImages.includes(imageElement.getAttribute('c2paId'))) {
...
export function handleSingleVideo(videoElement, singleImageVerification) {
  if (videoElement && !handledImages.includes(videoElement.getAttribute('c2paId'))) {
...
export function handleSingleAudio(audioElement, singleImageVerification) {
  if (audioElement && !handledImages.includes(audioElement.getAttribute('c2paId'))) {
    prepareAndHandleAudio(audioElement, singleImageVerification);
    visibleImages.push(audioElement);
  }
}
...
function handleAudios() {
  const audioElements = Array.from(document.querySelectorAll('audio')).filter((audio) => !handledImages.includes(audio.getAttribute('c2paId')));
...
export function removeC2PAIndicatorOnImgComponents() {
  ...
  // handledImages = [];
  // visibleImages = []; //done in removeC2PAIndicatorOnAudioComponents();
}
