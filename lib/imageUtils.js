 ...
 */
const addIconForImage = (image, baseId) => {
...
function prepareAndHandleImage(imageElement) {
  ...
  addIconForImage(imageElement, baseId);
...
const adjustIconPosition = (image) => {
  ...
  const popover = document.getElementById(`popover-${image.id}`);
  ...
  if (popover) {
    popover.placement = determinePopoverPlacementSide(image);
  }
};
