...
function getVisibleRect(element) {
  if (imageHasOverflow[element.id] === 'false') {
...
const fn = () => {
  list = [];
  list.push(...hiddenImages);
  list.push(...visibleImages);
  try {
    list.forEach((image) => {
      const rect = image.getBoundingClientRect();
      const topRightX = rect.left + rect.width - 30;
      const topRightY = rect.top + 2;
      console.log(rect, topRightX, topRightY)
      console.log(document.elementFromPoint(topRightX, topRightY), image)
      if (document.elementFromPoint(topRightX, topRightY) !== image
        && visibleImages.includes(image)) {
          // console.log('debug',`${image.id} is not visible`);
          if(!hiddenImages.includes(image)){
            hiddenImages.push(image);
          }
          makeIconInvisible(image);
          visibleImages.filter((img) => img !== image);
      } else if (document.elementFromPoint(topRightX, topRightY) === image && !visibleImages.includes(image)) {
        // console.log('debug',`${image.id} is visible`);
        visibleImages.push(image);
        showElementAndDescendants(image);
        hiddenImages.filter((img) => img !== image);
      }
    });
  } catch (error) {
    console.log(error)
 ...
 *
 */
export function handleSingleImage(imageElement) {
  if (imageElement) {
...
function handleImages() {
  ...
  /*const observer2 = new IntersectionObserver((entries) => {
    ...
    entries.forEach((entry) => {
      ...
      const topHidden = rect.top < 0;
      const bottomHidden = rect.bottom > viewportHeight;
      const topCloser = Math.abs(rect.top) < (rect.bottom - viewportHeight);

      console.log(topHidden, bottomHidden, topCloser, entry.isIntersecting);

      if (!entry.isIntersecting && ((topHidden && !bottomHidden)
        || (topHidden && bottomHidden && topCloser))
        && visibleImages.includes(entry.target)) {
        makeIconInvisible(entry.target);
      }

      if (entry.isIntersecting && !visibleImages.includes(entry.target)) {
        visibleImages.push(entry.target);
        showElementAndDescendants(entry.target);
      }
    });
