...
function getCodeCoverage(testCoverage, watch, coverageConfig) {
    ...
    if (!Number.isNaN(avgCoverage)) {
        ...
        entries.push(`Code coverage: ${nanocolors_1.bold(testCoverage.passed ? nanocolors_1.green(percent) : nanocolors_1.red(percent))}`);
    }
    if (!testCoverage.passed && coverageConfig.threshold) {
        coverageTypes.forEach(type => {
            if (testCoverage.summary[type].pct < coverageConfig.threshold[type]) {
                entries.push(`Coverage for ${nanocolors_1.bold(type)} failed with ${nanocolors_1.bold(nanocolors_1.red(`${testCoverage.summary[type].pct} %`))} compared to configured ${nanocolors_1.bold(`${coverageConfig.threshold[type]} %`)}`);
            }
        });
    }
    if (!watch && coverageConfig.report && ((_a = coverageConfig.reporters) === null || _a === void 0 ? void 0 : _a.includes('lcov'))) {
        entries.push(`View full coverage report at ${nanocolors_1.underline(path_1.default.join((_b = coverageConfig.reportDir) !== null && _b !== void 0 ? _b : '', 'lcov-report', 'index.html'))}`);
    }
    entries.push('');
    return entries;
}
