...
function renderDiff(actual, expected) {
    function cleanUp(line) {
        if (line[0] === '+') {
            return nanocolors_1.green(line);
        }
        if (line[0] === '-') {
            return nanocolors_1.red(line);
        }
        if (line.match(/@@/)) {
            return null;
        }
    ...
    const diffMsg = diff
        ...
        .filter(l => !!l)
        .join('\n');
    return `${nanocolors_1.green('+ expected')} ${nanocolors_1.red('- actual')}\n\n${diffMsg}`;
}
function formatError(error) {
    ...
    strings.push(nanocolors_1.red(errorMsg));
    if (showDiff) {
    ...
    if (error.stack) {
        if (showDiff) {
            const dedented = error.stack
                ...
                .map(s => s.trim())
                .join('\n');
            strings.push(nanocolors_1.gray(dedented));
        }
        else {
            strings.push(nanocolors_1.gray(error.stack));
        }
    }
    if (!error.expected && !error.stack) {
        strings.push(nanocolors_1.red(error.message || 'Unknown error'));
    }
    return strings.join('\n');
}
