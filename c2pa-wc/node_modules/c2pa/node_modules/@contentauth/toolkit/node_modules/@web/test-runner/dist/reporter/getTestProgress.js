...
function getProgressReport(name, minWidth, finishedFiles, activeFiles, testFiles, passedTests, skippedTests, failedTests) {
    ...
    const testResults = `${nanocolors_1.green(`${passedTests} passed`)}` +
        `, ${failedTests !== 0 ? nanocolors_1.red(failedText) : failedText}` +
        (skippedTests !== 0 ? `, ${nanocolors_1.gray(`${skippedTests} skipped`)}` : '');
...
function getTestProgressReport(config, args) {
    ...
    if (testRun !== -1 && unfinishedSessions.length === 0) {
        if (coverage && !testCoverage) {
            entries.push(nanocolors_1.bold('Calculating code coverage...'));
        }
        else if (config.watch) {
            entries.push(nanocolors_1.bold(`Finished running tests, watching for file changes...`));
        }
        else {
            ...
            if (failed) {
                if (coverage && !(testCoverage === null || testCoverage === void 0 ? void 0 : testCoverage.passed)) {
                    entries.push(nanocolors_1.bold(nanocolors_1.red(`Finished running tests in ${duration}s, failed to meet coverage threshold.`)));
                }
                else if (failedTestCount > 0) {
                    entries.push(nanocolors_1.bold(nanocolors_1.red(`Finished running tests in ${duration}s with ${failedTestCount} failed tests.`)));
                }
                else if (finishedFiles.size > 0) {
                    entries.push(nanocolors_1.bold(nanocolors_1.red(`Error while running tests.`)));
                }
                else {
                    entries.push(nanocolors_1.bold(nanocolors_1.red(`Failed to run any tests.`)));
                }
            }
            else {
                entries.push(nanocolors_1.bold(`Finished running tests in ${duration}s, all tests passed! 🎉`));
            }
        }
    }
    else {
        entries.push(nanocolors_1.bold('Running tests...'));
    }
    entries.push('');
    return entries;
}
