...
// Helper to load .onnx models as ArrayBuffer
async function loadModelAsArrayBuffer(filePath) {
  const response = await fetch(chrome.runtime.getURL(filePath)); // Access file from the extension package
  if (!response.ok) {
...
(async () => {
  for (const config of modelConfigs) {
    ...
    try {
      const modelBuffer = await loadModelAsArrayBuffer(`lib/trustmark/models/${config.fname}`);
      sessions[config.sessionVar] = await ort.InferenceSession.create(modelBuffer, { executionProviders: ['webgpu'] });
  ...
  let startTime = new Date();
  try {
    const modelBuffer = await loadModelAsArrayBuffer('lib/trustmark/models/resizer.onnx');
    session_resize = await ort.InferenceSession.create(modelBuffer, { executionProviders: ['wasm'] }); // cannot use GPU for this due to lack of antialias
    let timeElapsed = new Date() - startTime;
