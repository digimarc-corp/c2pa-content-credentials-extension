...
export function removeCaiComponents() {
  ...
  indicatorElements.forEach((element) => {
    if (!handledMedias.map((id) => `indicator-${id}`).includes(element.id)) {
      element.remove();
    }
  });

  manifestSummaryElements.forEach((element) => {
    if (!handledMedias.map((id) => `manifest-${id}`).includes(element.id)) {
      element.remove();
    }
  });
}
 ...
 * Assigns the base ID to the image and adds it to the handledMedias array. Sets the crossorigin
 ...
 */
function prepareAndHandleImage(imageElement, singleImageVerification, lookForWatermark) {
  ...
  if (!imageElement.complete) {
    imageElement.onload = () => { // we make sure image is loaded to get access to it
      setImageDataURI(imageElement).then((imageDataURI) => {
        ...
        getC2PAManifest(imageElement, addIconForImage, singleImageVerification, lookForWatermark);
      });
    };
    return;
  }
...
function prepareAndHandleVideo(videoElement, singleImageVerification, lookForWatermark) {
  let iconContainer = document.getElementById('icon-container');
  if (!iconContainer) {
    iconContainer = createIconContainer();
  }
  ...
  setImageDataURI(videoElement).then((imageDataURI) => {
    ...
    getC2PAManifest(videoElement, addIconForImage, singleImageVerification, lookForWatermark);
  });
}

function prepareAndHandleAudio(audioElement, singleImageVerification, lookForWatermark) {
  let iconContainer = document.getElementById('icon-container');
  if (!iconContainer) {
    iconContainer = createIconContainer();
  }
  const baseId = generateBaseId(audioElement);
  audioElement.setAttribute('c2paId', baseId);
  // videoElement.id = baseId;
  handledMedias.push(baseId);
  ...
  if (!audioElement.src) {
    ...
    setImageDataURI(audioElement).then((imageDataURI) => {
      audioElement.dataURI = imageDataURI;
      getC2PAManifest(audioElement, addIconForImage, singleImageVerification, lookForWatermark);
    });
  }
}
...
const makeIconInvisible = (image) => {
  visibleMedias = visibleMedias.filter((img) => img !== image);
  const icon = document.getElementById(`icon-${image.getAttribute('c2paId')}`);
  if (icon) {
 ...
 *
 */
export function handleSingleImage(imageElement, singleImageVerification, lookForWatermark) {
  if (imageElement && !handledMedias.includes(imageElement.getAttribute('c2paId'))) {
    prepareAndHandleImage(imageElement, singleImageVerification, lookForWatermark);
    visibleMedias.push(imageElement);
  }
}

export function handleSingleVideo(videoElement, singleImageVerification, lookForWatermark) {
  if (videoElement && !handledMedias.includes(videoElement.getAttribute('c2paId'))) {
    prepareAndHandleVideo(videoElement, singleImageVerification, lookForWatermark);
    visibleMedias.push(videoElement);
  }
}

export function handleSingleAudio(audioElement, singleImageVerification, lookForWatermark) {
  if (audioElement && !handledMedias.includes(audioElement.getAttribute('c2paId'))) {
    prepareAndHandleAudio(audioElement, singleImageVerification, lookForWatermark);
    visibleMedias.push(audioElement);
  }
}
...
async function handleImages() {
  // filter images based on size
  const imageElements = Array.from(document.querySelectorAll('img')).filter((img) => !handledMedias.includes(img.getAttribute('c2paId')));
    ...
    entries.forEach((entry) => {
      chrome.storage.local.get({ activated: false }, (result) => {
        ...
        if (entry.isIntersecting) {
          ...
          if (!visibleMedias.includes(entry.target)) {
            visibleMedias.push(entry.target);
            showElementAndDescendants(entry.target);
          }
        } else if (visibleMedias.includes(entry.target)) {
...
function handleVideos() {
  const videoElements = Array.from(document.querySelectorAll('video')).filter((video) => !handledMedias.includes(video.getAttribute('c2paId')));
    ...
    entries.forEach((entry) => {
      chrome.storage.local.get({ activated: false }, (result) => {
        ...
        if (entry.isIntersecting
          ...
          if (!handledMedias.includes(entry.target.getAttribute('c2paId'))) {
            prepareAndHandleVideo(entry.target, false, true);
          }
        } else if (entry.isIntersecting && result.activated) {
          // If the image is not in the handled list
          if (!handledMedias.includes(entry.target.getAttribute('c2paId'))) {
...
function handleAudios() {
  const audioElements = Array.from(document.querySelectorAll('audio')).filter((audio) => !handledMedias.includes(audio.getAttribute('c2paId')));

  const filteredAudioElements = filterImages(audioElements);
  ...
  // Observe each image element
  filteredAudioElements.forEach((audioElement) => {
    intersectionObserver.observe(audioElement);
  });
}
...
export function addC2PAIndicatorOnAudioComponents() {
  handleAudios();
...
export function addC2PAIndicatorOnAllComponents() {
  addC2PAIndicatorOnImgComponents();
  addC2PAIndicatorOnVideoComponents();
  addC2PAIndicatorOnAudioComponents();
}

/**
 * Revert the changes made to the images in the handledMedias list.
 */
export function removeC2PAIndicatorOnImgComponents() {
  handledMedias.forEach((id) => {
  ...
  // handledMedias = [];
  // visibleMedias = []; //done in removeC2PAIndicatorOnAudioComponents();
}
...
export function removeC2PAIndicatorOnAudioComponents() {
  handledMedias.forEach((id) => {
    // Find the original image
    // const image = document.getElementById(id);
    const image = document.querySelector(`audio[c2paId="${id}"]`);
